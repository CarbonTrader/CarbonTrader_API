...
def update_retire_date(transaction_type, serial, recipient_email, sender_email):
    ...
    if found_cp.exists:
        cp_projects = db.collection('CreditProvider').document(
            sender_email).collection('GreenProjects').stream()
...
class TransactionService:
    ...
    def build_transaction(transaction_data):
        transaction_data['public_key_sender'] = format_key(
            transaction_data['public_key_sender'])
        transaction_data['private_key_sender'] = format_key(
            transaction_data['private_key_sender'])
    ...
    def transfer_credits(recipient_response, sender_response, serial, trans):
        ...
        signature_aux = (int(trans["signature"][0]),
                         int(trans["signature"][1]))
        trans["signature"] = signature_aux
        db.collection("Seriales_En_Venta").document(
            trans['carbon_trader_serial']).delete()
        update_retire_date(
            trans['transaction_type'], trans['carbon_trader_serial'], recipient['email'], sender['email'])
    ...
    def multi_exchange(request):
        print(request)
        found_cp = db.collection('CreditProvider').document(
            request['cp_email']).get()
